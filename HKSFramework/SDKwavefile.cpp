...
HRESULT CWaveFile::Open(LPSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        if( NULL == m_hmmio )
        {
            ...
            if( NULL == ( hResInfo = FindResource( NULL, strFileName, "WAVE" ) ) )
            {
                if( NULL == ( hResInfo = FindResource( NULL, strFileName, "WAV" ) ) )
                    return DXTRACE_ERR( "FindResource", E_FAIL );
            }

            if( NULL == ( hResData = LoadResource( GetModuleHandle( NULL ), hResInfo ) ) )
                return DXTRACE_ERR( "LoadResource", E_FAIL );

            if( 0 == ( dwSize = SizeofResource( GetModuleHandle( NULL ), hResInfo ) ) )
                return DXTRACE_ERR( "SizeofResource", E_FAIL );

            if( NULL == ( pvRes = LockResource( hResData ) ) )
                return DXTRACE_ERR( "LockResource", E_FAIL );

            m_pResourceBuffer = new CHAR[ dwSize ];
            if( m_pResourceBuffer == NULL )
                return DXTRACE_ERR( "new", E_OUTOFMEMORY );
        ...
        if( FAILED( hr = ReadMMIO() ) )
        {
            ...
            mmioClose( m_hmmio, 0 );
            return DXTRACE_ERR( "ReadMMIO", hr );
        }

        if( FAILED( hr = ResetFile() ) )
            return DXTRACE_ERR( "ResetFile", hr );
    ...
    else
    {
        m_hmmio = mmioOpen( strFileName, NULL, MMIO_ALLOCBUF |
                            ...
                            MMIO_CREATE );
        if( NULL == m_hmmio )
            return DXTRACE_ERR( "mmioOpen", E_FAIL );

        if( FAILED( hr = WriteMMIO( pwfx ) ) )
        {
            mmioClose( m_hmmio, 0 );
            return DXTRACE_ERR( "WriteMMIO", hr );
        }
...
HRESULT CWaveFile::ReadMMIO()
{
    ...
    if( ( 0 != mmioDescend( m_hmmio, &m_ckRiff, NULL, 0 ) ) )
        return DXTRACE_ERR( "mmioDescend", E_FAIL );

    // Check to make sure this is a valid wave file
    if( ( m_ckRiff.ckid != FOURCC_RIFF ) ||
        ( m_ckRiff.fccType != mmioFOURCC( 'W', 'A', 'V', 'E' ) ) )
        return DXTRACE_ERR( "mmioFOURCC", E_FAIL );
    ...
    if( ckIn.cksize < ( LONG )sizeof( PCMWAVEFORMAT ) )
        return DXTRACE_ERR( "sizeof(PCMWAVEFORMAT)", E_FAIL );

    // Read the 'fmt ' chunk into <pcmWaveFormat>.
    if( mmioRead( m_hmmio, ( HPSTR )&pcmWaveFormat,
                  sizeof( pcmWaveFormat ) ) != sizeof( pcmWaveFormat ) )
        return DXTRACE_ERR( "mmioRead", E_FAIL );
    ...
    if( pcmWaveFormat.wf.wFormatTag == WAVE_FORMAT_PCM )
    {
        ...
        if( NULL == m_pwfx )
            return DXTRACE_ERR( "m_pwfx", E_FAIL );
    ...
    {
        SAFE_DELETE( m_pwfx );
        return DXTRACE_ERR( "mmioAscend", E_FAIL );
    }
    ...
    else
    {
        ...
        else
        {
            ...
            if( 0 != mmioCreateChunk( m_hmmio, &m_ck, 0 ) )
                return DXTRACE_ERR( "mmioCreateChunk", E_FAIL );
...
HRESULT CWaveFile::WriteMMIO( WAVEFORMATEX* pwfxDest )
{
    ...
    if( pwfxDest->wFormatTag == WAVE_FORMAT_PCM )
    {
        if( mmioWrite( m_hmmio, ( HPSTR )pwfxDest,
                       sizeof( PCMWAVEFORMAT ) ) != sizeof( PCMWAVEFORMAT ) )
            return DXTRACE_ERR( "mmioWrite", E_FAIL );
    }
    else
    {
